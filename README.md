...
This repository is organized into eight functional categories:
...
7. **Charge density analysis** — analyze and transform VASP CHGCAR volumetric data (slices, averages, STM simulation, supercells, charge-density differences), and plot/export VASPKIT `.grd` grid files
...
File format is auto-detected from the extension: `.hdf5` → HDF5; any other extension → Phonopy text format. Both input and output independently follow this rule. The cutoff radius for the hiPhive cluster space is set to the maximum cutoff supported by the supercell geometry minus a small margin (10⁻⁵ Å).
...
1. If `phonopy_params.yaml` is not present, runs `phonopy-init --fz vaspout-sposcar.h5 $(printf "%s\n" vaspout-*.h5 | grep -v '^vaspout-sposcar.h5$') --sp` to generate it
2. Runs `phonopy phonopy_params.yaml --writefc --full-fc` to write `FORCE_CONSTANTS`
...
Computes the RMS of each 3×3 2nd IFC block from a Phonopy `FORCE_CONSTANTS` file and pairs it with the corresponding minimum-image interatomic distance from a POSCAR. Useful for visualizing how 2nd IFC strength decays with distance for each element pair.
...
**Output:** `RMS_A-B.dat` per element pair — columns: distance (Å), 2nd IFC RMS (eV/Å2).

---

#### `compareIFCs.py`

Compares interatomic force constants (IFCs) between DFT and MLFF calculations by reading Phono3py HDF5 files and writing the residual (MLFF − DFT) to `.dat` files. Auto-detects whether the file contains 2nd (`force_constants`) or 3rd (`fc3`) IFCs.
...
**Output files:**
- `2ndIFCs.dat` — 2nd IFC comparison in eV/Å2
- `3rdIFCs.dat` — 3rd IFC comparison in eV/Å3
...
Q-point path distances (Å−1) are read from the second line of the input file. The frequency range is determined automatically as floor(f_min) to ceil(f_max) from all frequency values in the file. For each interior high-symmetry q-point, three coordinate pairs are written to trace a vertical tick from `fmin` to `fmax` and back. The outer box boundaries and the zero-frequency axis are appended at the end.

**Output:** `QLINES.dat` — columns: q-path distance (Å−1), frequency boundary (THz).

---

#### `convergePhono3py.py`

Checks the convergence of lattice thermal conductivity (κL) as a function of q-mesh density by reading multiple `kappa-mXXX.hdf5` files from the current directory.
...
**2D renormalization:** After loading the HDF5 files, the script interactively prompts for dimensionality (1 = 3D, 2 = 2D). For 2D materials, the vacuum direction is assumed to be c. A dimensionless renormalization factor derived from the c-axis length is applied to all κL values, correcting Phono3py's bulk-convention κL to the 2D-referenced value. Units remain W/(m·K) throughout.
...
Output filenames follow the pattern `<tag>-mXXXXXX.dat`, where the mesh token is preserved from the input filename (e.g., `kappa-m111111.hdf5` → `KappaVsT-m111111.dat`). All κL tensor components are written in Voigt notation (xx, yy, zz, yz, xz, xy) in W/(m·K).

**2D renormalization:** After loading the HDF5 file, the script interactively prompts for dimensionality (1 = 3D, 2 = 2D). For 2D materials, the vacuum direction is assumed to be c. A dimensionless renormalization factor derived from the c-axis length is applied to all κL arrays before any output is written, correcting Phono3py's bulk-convention κL to the 2D-referenced value. Units remain W/(m·K) throughout. The renormalization applies to all output file groups below.

**Temperature-dependent files** (one value per temperature row, written to the working directory):
- `KappaVsT.dat` / `Kappa_bandVsT.dat` — total κL tensor and band decomposition (3 acoustic + 1 summed optical) vs. temperature
- `ContributeKappaVsT.dat` — per-mode percentage contribution to total κL vs. temperature
...
- `Kappa_RTAVsT.dat` / `Kappa_RTA_bandVsT.dat` — RTA κL tensor and band decomposition vs. temperature (--lbte only)
- `Kappa_C*VsT.dat` — wave-like (coherence) Wigner κL tensor and band decomposition vs. temperature (--wigner only)
- `Kappa_P_RTA*VsT.dat` / `Kappa_TOT_RTA*VsT.dat` — particle-like and total Wigner κL (RTA) vs. temperature (--wigner --br only)
- `Kappa_P_exact*VsT.dat` / `Kappa_TOT_exact*VsT.dat` — particle-like and total Wigner κL (exact) vs. temperature (--wigner --lbte only)
...
- `KappaVsFrequency.dat` / `KappaVsMfp.dat` — mode κL vs. phonon frequency (THz) and vs. mean free path (Å)
- `cumulative_KappaVsFrequency.dat` / `cumulative_KappaVsMfp.dat` — cumulative κL sorted by ascending frequency and MFP
- `derivative_KappaVsFrequency.dat` / `derivative_KappaVsMfp.dat` — spectral κL density d(κL)/d(frequency) and d(κL)/d(MFP)
...
Temperature subdirectories (`T<value>K/`) are detected automatically from the working directory. All κL tensor components are written as the full 3×3 tensor (xx, xy, xz, yx, yy, yz, zx, zy, zz) in W/(m·K). All scattering rate and lifetime files are written per phonon branch. Phonon lifetimes are computed as τ = 1/(2 × 2π × Γ) (ps); modes with Γ ≤ 0 are assigned τ = 0.

**Temperature-dependent files** (one value per temperature row, written to the working directory):
- `Kappa_*VsT.dat` — total κL tensor vs. temperature, RTA and iterative (CONV) solutions
- `Kappa_bandVsT.dat` — κL tensor decomposed into 3 acoustic branches + 1 summed optical branch vs. temperature
- `HeatCapacityVsT.dat` — total heat capacity Cv (J/(m3·K)) vs. temperature
...
- `CumulativeKappaVsMFP.dat` / `CumulativeKappaVsFrequency.dat` — cumulative κL tensor vs. mean free path (Å) and vs. frequency (THz)
...
- Computes anisotropy indices: universal (AU), bulk (AB), shear (AG), and planar (A1, A2, A3)
...
Extracts the piezoelectric stress tensor (e, C/m2) and elastic stiffness tensor (C, GPa or N/m) from a VASP `OUTCAR` and computes the piezoelectric strain tensor (d = e·S, pm/V) via the compliance tensor S = C−1.
...
**`TWIST_LIST.dat` reuse:** After a search, results are written to `TWIST_LIST.dat` along with a structure snapshot (`original.vasp` for homobilayer; `bottom.vasp` + `top.vasp` for heterobilayer). On a later run with the same input file(s), the snapshot is compared against the provided POSCAR(s) (tolerance 10−6 Å); if they match, the saved candidate list is reused directly — skipping the search — and the script goes straight to candidate selection. If they don't match, a fresh search runs automatically with a warning.
...
Each configuration block is mapped to one extxyz frame with lattice, positions, energy, forces, and stress. Stress is converted from kbar to eV/Å3.
...
- `nebefs.dat` — extended table adding stress (kBar), volume (Å3), magnetic moment (μB) (SSNEB mode only)
...
Scripts for analyzing, transforming, and visualizing VASP volumetric charge-density data (CHGCAR/CHG/PARCHG) and VASPKIT-exported grids.

---

#### `vaspCharge.py`

Reads a CHGCAR-format file (CHG and PARCHG share the same layout) and performs one of ten charge-density analysis or transformation tasks, selected interactively. Non-collinear (LSORBIT/LNONCOLLINEAR) CHGCAR files are detected and rejected with an error (not yet supported).
...
1. Slice of Charge Density — 2D cross-section perpendicular to a chosen lattice axis at a chosen fractional coordinate, linearly interpolated between grid planes. Output: `SLICE_X.grd`, `SLICE_Y.grd`, `CHGSLICE.grd`
2. Charge Density — total electron density, unmodified. Output: `CHGTOT.vasp`
3. Spin Density — magnetization ρ_up − ρ_down (ISPIN = 2 only). Output: `CHGMAG.vasp`
4. Spin-Up & Spin-Down Density — derived densities (tot ± mag) / 2 (ISPIN = 2 only). Output: `CHGUP.vasp`, `CHGDW.vasp`
5. Linear-Average Charge Density — density averaged over one axis, leaving a 2D map. Output: `LAVG_X.grd`, `LAVG_Y.grd`, `CHGLAVG.grd` — ready for `plotGrid.py`
6. Planar-Average Charge Density — 1D profile along a chosen axis. Output: `PLANAR_AVERAGE.dat`
7. Macroscopic-Average Charge Density — the planar-average profile further smoothed with a periodic moving-window average (FFT convolution), for work-function/band-alignment analysis; window length should normally equal one interplanar spacing. Output: `MACROSCOPIC_AVERAGE.dat`
8. Charge Density Along a Specified Path — trilinear-interpolated density along a line between two fractional points. Output: `CHG_LINE.dat`
9. Build Supercell of Charge Density — tiles the density grid and structure by integer (n1, n2, n3) multiples; tiled values are rescaled by n1×n2×n3 since CHGCAR stores ρ(r)·V_cell. Output: `CHGCAR_SUPERCELL.vasp`
10. STM Simulation — Tersoff-Hamann-style image from the density (e.g. a PARCHG near E_F), assuming the surface normal is axis C with vacuum on the +C side. Constant-height mode reports I(x,y) at a fixed height; constant-current mode reports the height z(x,y) where density first crosses a chosen threshold, scanning in from vacuum. Output: `STM_X.grd`, `STM_Y.grd`, and `STM_HEIGHT.grd` or `STM_CURRENT.grd`

**Output convention:** `.vasp` outputs are written in VASP's native ρ(r)·V_cell convention with no PAW augmentation-occupancies block — valid for visualization (e.g. VESTA) but not for restarting VASP. `.grd` outputs are plain whitespace-delimited grids, readable by `plotGrid.py`.

---

#### `vaspChargeDiff.py`

Computes a charge-density difference Δρ = ρ_AB − ρ_A − ρ_B from three CHGCAR files — a combined system and its two isolated fragments in the same cell — e.g. for analyzing electron transfer in an adsorbate + substrate system.

```
Usage: vaspChargeDiff.py <CHGCAR_AB> <CHGCAR_A> <CHGCAR_B>
```

All three files must share identical grid dimensions (hard error on mismatch). A lattice-vector mismatch between `CHGCAR_AB` and either fragment (tolerance 1e-4 Å) only warns, since the difference may still be informative.

**Output:** `CHGDIFF.vasp` — same ρ(r)·V_cell convention as `vaspCharge.py`.

---

#### `plotGrid.py`

Reads a VASPKIT-generated 2D grid — `X.grd`, `Y.grd`, and a value grid such as `CHGLAVG.grd` (planar-averaged charge density) — and produces a filled-contour heatmap plus an ASCII export for external plotting.

```
Usage: plotGrid.py <X.grd> <Y.grd> <CHGLAVG.grd> [output prefix]
```

If [output prefix] is omitted, the value-grid filename (without extension) is used, e.g. `CHGLAVG.grd` → `CHGLAVG.png` / `CHGLAVG.dat`.

**Grid validation:** `X`, `Y`, and the value grid must share identical shape (hard error on mismatch, since a shape mismatch would silently truncate the arrays when they are flattened and zipped together for the ASCII export). Axis alignment — X constant along rows, Y constant along columns, i.e. a regular rectilinear mesh — is checked with `atol=2e-4`, twice VASPKIT's 4-decimal-place output resolution, and only warns rather than errors, since both the contour plot and the ASCII export handle a non-rectilinear grid correctly.

**Interactive axis labeling:** Before plotting, the script always prompts (mandatory, non-empty, restricted to X/Y/Z) for which physical axis each plot axis represents, and labels it in Å with mathtext formatting.

**Plot:** filled contour (viridis colormap, 100 levels, equal aspect ratio) with a colorbar labeled for charge density (e/Å3 by default — edit the `CHGLAVG_LABEL` constant for other quantities), rendered in Times New Roman/serif per the toolkit's plot styling convention, saved at 300 dpi.
