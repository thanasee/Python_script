...
def write_POSCAR(filepath, lattice_matrix, elements, atom_counts, positions_cartesian,
    ...
    with open(filepath, 'w') as o:
        o.write("Generated by vaspMirror.py\n")
        o.write(f"   {1.0:.14f}\n")
...
def reflect(positions_cartesian):
    """Reflect atomic positions in Cartesian space across a user-chosen plane.

    Prompts the user to select a reflection plane (XY, XZ, or YZ) and negates
    the corresponding Cartesian component of every atomic position.

    Reflection axes:
        XY plane → negate Z (index 2)
        XZ plane → negate Y (index 1)
        YZ plane → negate X (index 0)
...
def main():
    ...
    write_POSCAR(argv[2], unmirror["lattice_matrix"], mapping["elements"], mapping["atom_counts"],
                 mapping["positions_cartesian"], mapping["positions_direct"], unmirror["selective_dynamics"],
