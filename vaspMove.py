...
def usage():
    ...
    print("""
Usage: vaspMove.py <input> <output>

Move selected atoms within a VASP POSCAR structure.
...
def write_POSCAR(filepath, lattice_matrix, elements, atom_counts, positions_cartesian,
    ...
    with open(filepath, 'w') as o:
        o.write("Generated by vaspMove.py\n")
        o.write(f"   {1.0:.14f}\n")
...
def select_index(total_atoms, species):
    """Parse free-format atom selection and return 0-based indexes.
    ...
    while True:
        ...
        for select in input().split():
            ...
            if select.isnumeric() or '-' in select:
                ...
                selected_atoms.extend([i for i, lbl in enumerate(species) if lbl == select])
        if selected_atoms and all(0 <= idx < total_atoms for idx in selected_atoms):
            return selected_atoms
        print("Wrong input atom-indexes! TRY AGAIN!")


def move_atoms(lattice_matrix, total_atoms, positions_cartesian, species):
    """Interactively move selected atoms by displacement or to absolute coordinates.

    Prompts for:
      1. Atom selection (index, range, element symbol, or 'all')
      2. Move mode:
           1) Displace by a (dx, dy, dz) vector — applied additively
           2) Move to absolute coordinates — centroid of selected atoms
              is translated to the target point
      3. Coordinate system: Cartesian (Å) or Direct (fractional)
      4. The displacement vector or target coordinates
    ...
    Returns
    -------
    np.ndarray (N, 3) — updated Cartesian positions
    """
    selected_atoms = select_index(total_atoms, species)

    print("""
Choices of move mode
1) Displace by vector (dx dy dz)
2) Move to absolute coordinates (x y z)""")

    while True:
        mode = input("Enter Choice: ")
        if mode in ('1', '2'):
            break
        print("ERROR!! Must choose mode 1 or 2")

    print("""
Coordinate system
1) Cartesian (Angstrom)
2) Direct (fractional)""")

    while True:
        coord = input("Enter Choice: ")
        if coord in ('1', '2'):
            break
    ...
    if mode == '1':
        label = "da db dc (fractional)" if use_direct else "dx dy dz (Angstrom)"
        print(f"Enter displacement vector {label} (separate by space):")
        while True:
            try:
        ...
        displacement = vec @ lattice_matrix if use_direct else vec
        new_positions_cartesian = positions_cartesian.copy()
        new_positions_cartesian[selected_atoms] += displacement

    else:
        label = "a b c (fractional)" if use_direct else "x y z (Angstrom)"
        print(f"Enter target coordinates {label} (separate by space):")
        while True:
            try:
                values = list(map(float, input().split()))
                if len(values) == 3:
                ...
                print("ERROR! Non-numeric input. Try again.")
        target = direct_to_cartesian(lattice_matrix, vec) if use_direct else vec
        centroid = np.mean(positions_cartesian[selected_atoms], axis=0)
        new_positions_cartesian = positions_cartesian.copy()
        new_positions_cartesian[selected_atoms] += target - centroid

    return new_positions_cartesian


def main():
    """Parse arguments, move selected atoms with selected method, and write output"""

    if '-h' in argv or '--help' in argv or len(argv) != 3:
        usage()

    poscar = read_POSCAR(argv[1])
    new_positions_cartesian = move_atoms(poscar['lattice_matrix'], poscar['total_atoms'],
                                         poscar['positions_cartesian'], poscar['species'])
    new_positions_direct = cartesian_to_direct(poscar['lattice_matrix'], new_positions_cartesian)
    mapping = mapping_elements(poscar['elements'], poscar['atom_counts'], new_positions_cartesian, new_positions_direct,
                               poscar['species'], poscar['selective_dynamics'], poscar['flags'])
    labels = define_labels(mapping['elements'], mapping['atom_counts'])
    write_POSCAR(argv[2], poscar['lattice_matrix'], mapping['elements'], mapping['atom_counts'], mapping['positions_cartesian'],
                 mapping['positions_direct'], poscar['selective_dynamics'], mapping['flags'], labels)
