...
def write_POSCAR(filepath, lattice_matrix, elements, atom_counts, positions_cartesian,
    ...
    with open(filepath, 'w') as o:
        o.write("Generated by vaspStrain.py\n")
        o.write(f"   {1.0:.14f}\n")
...
def applying_strain(lattice_matrix, positions_direct):
    """
    Prompt the user to input a strain matrix and apply it to the lattice matrix.

    The strain matrix is symmetrized before application. The new lattice is
    computed as:

        new_lattice = lattice @ (I + strain)

    Parameters
    ----------
    lattice_matrix : np.ndarray, shape (3, 3)
        The original lattice matrix in Angstroms.
    positions_direct : np.ndarray, shape (N, 3)
        The original fractional coordinates.

    Returns
    -------
    dict with keys:
        "strain_matrix" : np.ndarray, shape (3, 3)
            The symmetrized strain matrix (dimensionless).
        "lattice_matrix" : np.ndarray, shape (3, 3)
            The strained lattice matrix in Angstroms.
        "positions_cartesian" : np.ndarray, shape (N, 3)
            The Cartesian coordinates of the atoms in the strained lattice.
    """
...
Enter strain matrix (separate by space):"""
    ...
    new_lattice_matrix = lattice_matrix @ (np.eye(len(strain_matrix)) + strain_matrix)
    new_positions_cartesian = direct_to_cartesian(new_lattice_matrix, positions_direct)
...
def main():
    ...
    strained = applying_strain(unstrain["lattice_matrix"], unstrain["positions_direct"])
    mapping = mapping_elements(unstrain["elements"], unstrain["atom_counts"], strained["positions_cartesian"], unstrain["positions_direct"],
                               unstrain["species"], unstrain["selective_dynamics"], unstrain["flags"])
    labels = define_labels(mapping["elements"], mapping["atom_counts"])
    write_POSCAR(argv[2], strained["lattice_matrix"], mapping["elements"], mapping["atom_counts"],
                 mapping["positions_cartesian"], mapping["positions_direct"], unstrain["selective_dynamics"],
